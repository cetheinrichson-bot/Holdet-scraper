...
const rounds = {
  ...
  r5: { label:"Runde 5", start:"2026-08-23T12:00:00+02:00", end:"2026-08-24T21:00:00+02:00", waiverEnd:"2026-08-25T09:00:00+02:00" },
...
async function run() {
  ...
  for (const [k,r] of Object.entries(rounds)) {
    ...
    console.log("  "+k+" ["+r.status+"] "+r.start.slice(0,16)+" -> "+r.end.slice(0,16)+
                (r.status==="active"?"   <-- AKTIV":""));
  }
  console.log("\nLIGE NU:");
  console.log("  Pointscraping : " + (aktiv ? aktiv+" er aktiv" : "ingen aktiv runde"));
  console.log("  Waiver-periode: " + (waiver ? "JA ("+waiver+")" : "NEJ - spillere er frie"));
  process.exit(0);
}
